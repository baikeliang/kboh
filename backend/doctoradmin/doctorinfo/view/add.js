			                ...
			                <div className="main-input">
			                    ...
			                    <div className="input-box h30">
			                        ...
			                        <div className="radio-box" id="sex">
			                            <span><input type="radio" className="radio1" name="radio2" id="doctor_checkbox10" value="1" checked={sex=='1'?'checked':''}/><label style={{top:'6px'}} htmlFor="checkbox10" onClick={(e)=>{ click(e,'sex','1')}}></label>男</span>
			                            <span><input type="radio" className="radio1" name="radio2" id="doctor_checkbox11" value="2" checked={sex=='2'?'checked':''}/><label style={{top:'6px'}} htmlFor="checkbox11" onClick={(e)=>{ click(e,'sex','2')}}></label>女</span>
			                        </div>
			                        <p className="errorTip"></p>
			                    </div>
			                    <div className="input-box h30">
			                          ...
			                          <div className="checkBone" id="service_ids">
			                            {
			                            	projects?projects.map((project)=>{
			                            			var check = 0;
			                            		    service_name_arr?service_name_arr.map((pro)=>{
			                            		   	    if(pro[project.id]){
			                            		   	  	  check = 1;
			                            		   	    }
			                            		    }):''
		                                           return (<span style={{float:'left',paddingRight: '15px',paddingLeft: '15px',position: 'relative',lineHeight:'30px',fontSize:'12px'}}>
			                                                <input type="checkbox" checked={ check==1?'check':'' } className="checkbox" id={ 'checkbox_n_' + project.id  } value={ project.id }/>
			                                                <label onClick={ ()=>{ proClick(project.id,project.name,check)} } htmlFor={ 'checkbox_n_' + project.id  } style={{backgroundPositionY:'1px',lineHeight:'30px',paddingLeft:'19px',width: '16px',display: 'block',borderRadius: '2px',position:'absolute',top:'6px',left: '0'}}>
			                                                </label>
			                                               {project.name}
			                                              </span>) 
			                                        }):''}
