...
(function( $ ) {
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('uploadSuccess',function( file, response ){
				createBox( $fileInput, file, webUploader,response.file_path )
				var $diyBar = $('.diyBar');	
				setTimeout(function(){
                    $('.opcity-progress').hide();
                },600);
			...
			//选择文件错误触发事件;
			webUploader.on('error', function( error ) {
				var text = '';
				if(error == 'F_DUPLICATE'){
					$('.opacity-tip').fadeIn(500).delay(2000).fadeOut()
					$('.opacity-tip p').html('该文件已经被选择了').css({'margin-left':-($('.opacity-tip p').width()/2)})
					//alert('该文件已经被选择了')
				}else if(error == 'Q_EXCEED_NUM_LIMIT'){
					$('.opacity-tip').fadeIn(500).delay(2000).fadeOut()
					$('.opacity-tip p').html('上传文件数量超过限制').css({'margin-left':-($('.opacity-tip p').width()/2)})
					//alert('上传文件数量超过限制')
				}else if(error == 'F_EXCEED_SIZE'){
					$('.opacity-tip').fadeIn(500).delay(2000).fadeOut()
					$('.opacity-tip p').html('抱歉，您上传文件大小超过15MB').css({'margin-left':-($('.opacity-tip p').width()/2)})
					//alert('文件大小超过限制')
				}else if(error == 'Q_EXCEED_SIZE_LIMIT'){
					$('.opacity-tip').fadeIn(500).delay(2000).fadeOut()
					$('.opacity-tip p').html('所有文件总大小超过限制').css({'margin-left':-($('.opacity-tip p').width()/2)})
					//alert('所有文件总大小超过限制')
				}else if(error == 'Q_TYPE_DENIED'){
					$('.opacity-tip').fadeIn(500).delay(2000).fadeOut()
					$('.opacity-tip p').html('上传格式不正确').css({'margin-left':-($('.opacity-tip p').width()/2)})
					//alert('文件类型不正确或者是空文件')
				}else{
					$('.opacity-tip').fadeIn(500).delay(2000).fadeOut()
					$('.opacity-tip p').html('未知错误').css({'margin-left':-($('.opacity-tip p').width()/2)})
					//alert('未知错误')
				}
				/*switch( code ) {
	...
	function getOption(objId) {
		...
		*/
		return {
			...
			fileNumLimit:50,
			fileSizeLimit:5242880 * 100,
			fileSingleSizeLimit:5242880 * 3
		};
	}
	...
	function getUploader( opt ) {
		...
		return new window.WebUploader.Uploader( opt );;
	}
	...
	//创建文件操作div;	
	function createBox( $fileInput, file, webUploader,file_path ) {
		...
		//添加子容器;
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover" file_path="'+file_path+'"> \
		...
		var $diyCancel = $fileBox.children('.diyCancel').one('click',function(){
			...
			//console.log(file_id+'/'+webUploader)
		});
